        ...
        <>
            ...
            <aside className={`${classes} flex flex-col h-full`}>
                ...
                <div className="p-4 border-t bg-slate-50 flex items-center gap-3">
                    <img src={user?.photo || "https://ui-avatars.com/api/?name=" + user.name} className="w-10 h-10 rounded-full border border-white shadow-sm object-cover" alt="Profile" />
                    <div className="flex-1 cursor-pointer overflow-hidden" onClick={() => {setTab('profile'); closeMenu();}}>
                        <p className="text-sm font-bold text-slate-700 truncate">{user?.name}</p>
                        <p className="text-xs text-slate-400 capitalize">{user?.role}</p>
                    </div>
                    <button onClick={onLogout} className="text-red-400 hover:text-red-600 p-2 rounded-lg hover:bg-red-50">
